...
void print_ctx(blake2s_ctx *ctx)
{
	...
	printf("0x%08x, 0x%08x, 0x%08x, 0x%08x, ", ctx->h[0], ctx->h[1],
	       ctx->h[2], ctx->h[3]);
	printf("0x%08x, 0x%08x, 0x%08x, 0x%08x", ctx->h[4], ctx->h[5],
	       ctx->h[6], ctx->h[7]);
	printf("\n");
...
//------------------------------------------------------------------
void G(uint32_t *v, uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t x,
       uint32_t y)
{
	if (VERBOSE) {
		printf("G started.\n");
...
//------------------------------------------------------------------
void blake2s_update(blake2s_ctx *ctx, const void *in,
